...
<template>
  <ElDialog
    ...
    :title="type === 'add' ? '新增积分变动日志' : '编辑积分变动日志'"
    width="600px"
  ...
  >
    <ElForm ref="formRef" :model="formData" :rules="rules" label-width="100px">
      ...
      <ElFormItem label="变动积分" prop="score">
        <ElInputNumber v-model="formData.score" :min="0" controls-position="right" />
      </ElFormItem>
      <ElFormItem label="变动前积分" prop="before">
        <ElInputNumber v-model="formData.before" :min="0" controls-position="right" />
      </ElFormItem>
      <ElFormItem label="变动后积分" prop="after">
...
<script setup lang="ts">
  ...
  const rules = reactive<FormRules>({
    ...
    score: [{ required: true, message: '变动积分不能为空', trigger: 'blur' }],
    before: [{ required: true, message: '变动前积分不能为空', trigger: 'blur' }],
    after: [{ required: true, message: '变动后积分不能为空', trigger: 'blur' }],
