<!-- 悠然示例 详情抽屉 -->
<template>
  <ElDrawer v-model="visible" title="悠然示例详情" size="600px" :destroy-on-close="true">
    ...
    <ElDescriptions v-else-if="detail" :column="1" border>
      ...
      <ElDescriptionsItem label="标题">{{ detail.tname ?? '-' }}</ElDescriptionsItem>
      <ElDescriptionsItem label="图片">
        <ElImage
          ...
          fit="cover"
          :preview-src-list="[detail.timage]"
        />
        <span v-else>-</span>
      </ElDescriptionsItem>
      <ElDescriptionsItem label="测试">{{ detail.ttest ?? '-' }}</ElDescriptionsItem>
    </ElDescriptions>
  </ElDrawer>
</template>
...
<script setup lang="ts">
  ...
  import { fetchUranExampleView } from '@/api/backend/uran-example'
  ...
  watch(visible, async (val) => {
    if (val && props.viewId) {
      loading.value = true
      try {
        detail.value = (await fetchUranExampleView(props.viewId)) as any
      } catch {
        detail.value = null
      }
      loading.value = false
