...
<script setup lang="ts">
  ...
  // 递归过滤 hidden 菜单
  const filterHiddenMenus = (menus: any[]): any[] => {
    return menus
      .filter((m: any) => !m.meta?.isHide)
      .map((m: any) => m.children ? { ...m, children: filterHiddenMenus(m.children) } : m)
  }

  const menuList = computed(() => {
    const menuStore = useMenuStore()
    const allMenus = filterHiddenMenus(menuStore.menuList)
