...
<template>
  <section v-if="hasVisibleItems" class="art-search-bar art-card-xs" :class="{ 'is-expanded': isExpanded }">
    <ElForm
      ref="formRef"
      :model="modelValue"
...
<script setup lang="ts">
  ...
  /**
   * 是否有可见的搜索项（没有则隐藏整个搜索栏）
   */
  const hasVisibleItems = computed(() => {
    return props.items.some((item) => !item.hidden)
  })
